...
import {
  ArrowLeft, ArrowRight, CheckCircle2, Clock, PartyPopper,
  ChevronRight, X, Award, Sparkles,
} from "lucide-react";
  ...
  return (
    <div className="space-y-4 text-center">
      <div className="w-20 h-20 rounded-3xl bg-green-100 flex items-center justify-center mx-auto">
        <Sparkles className="w-10 h-10 text-[#145C25]" />
      </div>
          ...
          {q.options.map((opt, i) => (
            ...
            >
              ...
              {selected === i && selected === q.correct && (
                <CheckCircle2 className="w-5 h-5 text-green-500 inline ml-2" />
              )}
            </button>
          ))}
        </div>
    ...
    <div className="space-y-6 text-center py-8">
      <div className="relative">
        <div className="w-24 h-24 rounded-full bg-green-100 flex items-center justify-center mx-auto">
          <PartyPopper className="w-12 h-12 text-[#145C25]" />
        </div>
      ...
      <div className="bg-green-50 rounded-2xl p-4 inline-block">
        <p className="text-green-700 font-bold flex items-center gap-2">
          <Award className="w-5 h-5" /> +500 EcoPoints Bonus Earned!
        </p>
      </div>
      <Button
        onClick={onFinish}
        ...
        <div className="px-6 py-4 border-t border-neutral-100">
          <div className="flex items-center gap-3 max-w-lg mx-auto">
            ...
            >
              ...
              {!completing && <ArrowRight className="w-4 h-4 ml-2" />}
            </Button>
          </div>
        </div>
      )}
