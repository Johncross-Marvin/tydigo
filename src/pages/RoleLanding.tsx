 ...
 *
 * Authentication-intent CTAs open the shared global auth dialog (with the
 * correct canonical account type) instead of rendering an inline form near
 * the footer. This keeps the conversion surface in the viewport and avoids
 * a duplicated signup form drifting from the shared implementation.
 */

import { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import {
  ArrowLeft, ArrowRight, CheckCircle2, Recycle, Home, Building2, BarChart3,
  Truck, Leaf, Globe, Shield, Clock, DollarSign, Award, Package, Users,
  Calendar, FileText, MapPin, ChevronDown,
} from "lucide-react";
...
const RoleLandingPage = () => {
  ...
  const canonicalRole = config.role as PublicAccountType;
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <AuthTrigger
              mode="signin"
              source={`${canonicalRole}-nav-signin`}
              className="inline-flex items-center justify-center h-10 px-4 rounded-xl border border-neutral-200 text-sm font-semibold text-neutral-700 hover:bg-neutral-100"
            >
              Sign In
            </AuthTrigger>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <AuthTrigger
                  ...
                  source={`${canonicalRole}-hero`}
                  className="inline-flex items-center justify-center bg-[#145C25] hover:bg-[#0F4A1E] text-white font-bold text-base px-8 py-6 rounded-2xl shadow-brand"
                >
                  Get Started
      ...
      {/* ── Final CTA (opens shared dialog, no inline form) ── */}
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-lg mx-auto px-4 sm:px-6 text-center">
          ...
          <AuthTrigger
            mode="signup"
            accountType={canonicalRole}
            source={`${canonicalRole}-footer-cta`}
            className="inline-flex items-center justify-center bg-[#145C25] hover:bg-[#0F4A1E] text-white font-bold text-base px-10 py-6 rounded-2xl shadow-brand"
          >
